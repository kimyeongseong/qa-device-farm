...
- **텍스트 입력은 양쪽 다 ASCII만** 됩니다. 안드로이드는 기기에 IME가 필요하고,
  iOS는 맥이 US 배열 키코드로 한 글자씩 치기 때문입니다. 한글을 넣어야 하면
  400이 돌아오니 다른 방법을 찾으세요.
...
- **iOS 미러링이 끊기면 사람이 붙어야 합니다.** `health`의 `ios.connection`이
  `blocked`/`no-window`/`not-running`이면 맥에서 직접 연결해야 하고("사용 중"이면
  아이폰을 잠가야 재개됩니다), 에이전트가 연결 화면을 눌러 통과할 수 없습니다.
  그 상태로 재시도하지 말고 사용자에게 알리세요.
