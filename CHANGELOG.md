...
에이전트가 팜을 직접 조작할 수 있게 하고, iOS를 범위에 넣고, 설치 없이 건네줄 수
있는 배포본을 만들었습니다. 나머지는 실기기와 브라우저로 직접 돌려보다 나온
수정입니다.

**받아서 바로 쓰기** — 아래 릴리즈 첨부에서 자기 OS의 zip을 받아 압축을 풀고
`시작하기.command`(맥) 또는 `시작하기.bat`(윈도우)을 실행하면 됩니다. 파이썬과
Node가 안에 있어 따로 설치할 필요가 없습니다. adb만 라이선스상 빠져 있어서
직접 넣어야 합니다(안내는 배포본 안 `먼저-읽어주세요.txt`에).

### 추가 (Added)

- **맥·윈도우 배포본** — 지금까지 이 팜을 남에게 주려면 "파이썬 깔고, Node 깔고,
  npm install 하고, 명령 두 개 실행"을 같이 줘야 했습니다. QA 도구가 안 퍼지는
  이유의 대부분이 그겁니다. `build/build.py`가 파이썬 런타임·Node 런타임·빌드한
  ws-scrcpy를 한 폴더에 넣어서, 받는 사람은 압축을 풀고 `시작하기.command`(맥) 또는
  `시작하기.bat`(윈도우)만 누르면 됩니다. 크로스 컴파일이 불가능하므로(PyInstaller는
  자기가 도는 OS용만 만듭니다) GitHub Actions의 macos-14(애플 실리콘)·macos-13(인텔)·
  windows-latest에서 각각 빌드하고, 그 앞에 테스트를 먼저 돌립니다 — 깨진 걸 빌드해서
  넘기는 것보다 안 넘기는 게 낫습니다. adb와 scrcpy는 라이선스상 여전히 빼고, 빈
  `scrcpy_bin/`과 어디서 받는지 적은 안내를 대신 넣습니다.
- **`DEVICE_FARM_HOME`** — 팜이 쓰는 파일(점유·매크로·별칭·로그)을 어디에 둘지
  지정합니다. 배포본을 만들면서 필요해졌습니다: 대시보드처럼 읽기만 하는 것은 번들
  안에 있어도 되지만(PyInstaller가 매 실행 임시 폴더에 풀고 끝나면 지웁니다), 재시작
  후에도 남아야 하는 것은 그럴 수 없습니다. 설정하지 않으면 지금까지와 똑같이
  현재 디렉터리를 씁니다.
- **AI 에이전트 조작 모드** — 기존 조작 API는 좌표로 말합니다. 사람한테는 맞는 인터페이스인데
  에이전트를 붙이면 "무엇을 누를지"는 이미 아는데 "그게 몇 픽셀인지"를 매번 알아내야 하고,
  스크린샷을 눈대중해서 찍은 좌표는 화면이 조금만 달라져도 틀립니다. 화면에 보이는 것으로
  말하는 `/api/agent/*`를 따로 뒀습니다 — `elements`(화면의 글자와 좌표), `tap-text`,
  `type-text`, `open-app`, `wait-stable`. **원격 조작 경로는 그대로**이고 그 옆에 별도
  모드로 붙습니다. 안드로이드의 화면 읽기는 OCR이 아니라 `uiautomator dump`입니다 — 기기가
  스스로 아는 뷰 트리라 인식 오류가 없고 의존성도 늘지 않습니다(대신 WebView·게임·
  `FLAG_SECURE` 화면은 비어 보이고, 그때는 스크린샷과 좌표로 내려가야 합니다).
- **`cli.py harness` — 여러 동작을 한 번에** — phone-harness의 heredoc과 같은 사용감으로
  `open_app` / `tap_text` / `wait_stable` 같은 줄을 이어서 실행합니다. 다만 **파이썬을
  실행하지 않습니다**: 동사 표에 있는 줄만 HTTP 호출로 바꾸고, 표에 없는 줄이 하나라도 있으면
  한 줄도 실행하기 전에 종료 코드 2로 거절합니다. 이 스크립트는 대개 LLM이 쓰고 팜은 실기기
  여러 대에 물려 네트워크에 열려 있어서, 임의 코드 실행 경로를 열어둘 이유가 없습니다.
  `--occupy --release`로 점유·반납을 대신합니다.
- **`cli.py skill` — Claude Code / Codex 스킬 등록** — 에이전트가 이 팜을 알아서 쓰도록
  SKILL.md를 만들어 `~/.claude/skills/` 또는 `~/.codex/skills/`에 씁니다. 점유·반납 규칙,
  동사 표, 플랫폼별 한계가 들어갑니다.
- **iOS 미러링 지원 (macOS 전용)** — 맥에 연결된 아이폰을 팜의 기기 한 대(`ios-mirror`)로
  다룹니다. 안드로이드와 마찬가지로 **원격 조작(`/static/ios.html`)과 AI 조작이 따로**
  있습니다. 화면은 스크린샷 폴링(ws-scrcpy는 안드로이드 전용), 조작과 OCR은
  [phone-harness](https://github.com/ShawnPana/phone-harness)에 subprocess로 위임합니다 —
  pyobjc를 이 프로세스로 끌어들이면 그쪽 임포트 실패 하나가 팜 전체를 못 뜨게 만듭니다.
  스크립트 템플릿은 상수이고 값은 환경변수로만 넘어갑니다(값이 파이썬 코드가 되지 않도록).
  **맥이 아니거나 phone-harness가 없으면 아이폰 칸이 아예 나타나지 않고** 나머지는 그대로
  동작합니다. 왜 없는지는 `/api/health`의 `ios.reason`과 기동 배너가 말합니다.
  좌표계는 경계에서 맞춥니다 — phone-harness는 맥 화면 좌표로 말하고 팜은 스크린샷 이미지
  픽셀로 말하는데, 레티나 미러링 창에서 두 값은 2배 차이가 나고 창을 옮기면 원점도 달라집니다.
  변환은 서버가 캐시한 값이 아니라 harness 스크립트 안의 `screen_info()`로 매번 계산합니다.
- **미러링이 끊긴 이유를 기기 목록에 표시** — phone-harness는 연결 대기 화면을 대신 눌러주지
  않습니다(재연결은 사람이 해야 하는 물리 동작이라는 판단). 팜도 그 판단을 따르고, 대신
  `connection_state()`를 읽어 안드로이드의 `unauthorized`와 같은 자리에 사유를 적습니다.
  이 확인은 캡처 + OCR이라 기기 목록 폴링(2초)에는 달지 않고, `/api/health`에서 TTL 30초로
  확인한 값을 목록이 재사용합니다.
- **`/api/device/{serial}/screenshot?full=1`** — 원본 해상도 PNG. 기본 응답은 300x600
  썸네일인데, 그걸로 좌표를 계산하는 에이전트는 1080x2400 기기에서 매번 엉뚱한 데를 누릅니다.
...
- **공백이 있는 텍스트를 입력하면 앞 단어만 들어가던 문제** — `input text`에 문자열을 argv
  하나로 넘기고 있었는데, adb는 명령을 문자열로 재조립해 기기에 보내고 **기기 셸이 그걸 다시
  쪼갭니다.** `"hello world"`가 기기에서 `input text hello world`가 되어 "hello"만
  입력됐습니다. 같은 경로가 주입 통로이기도 했습니다 — `a; reboot`이 기기 셸에서 명령
  두 개였습니다(서버 쪽 argv 규칙은 지켜졌지만 기기 셸에서 한 겹 더 풀렸습니다). 이제 기기
  셸용으로 인용부호를 씌우고 공백을 `%s`로 바꿔서 넘깁니다. 비ASCII가 400인 것은 그대로입니다.
...
- **로그 탭의 `[중지]`가 아무 일도 안 하는 것처럼 보이던 문제** — `/api/logcat`는 `capturing`을
  돌려주는데 모달이 그걸 무시하고 요약줄에 늘 `● 수집 중`을 찍고 있었습니다. 중지 후에도
  같은 문장이 남고 2초 폴링도 계속 돌아서, 멈춘 건지 버튼이 죽은 건지 화면으로 구분할 수가
  없습니다. 이제 `■ 중지됨 (버퍼는 남아 있음)`으로 상태를 그대로 표시하고, 수집이 끝나면
  폴링을 멈춥니다(`[수집 시작]`이 다시 켭니다). 수집 전에는 빈 화면이 아니라 `불러오는 중...`을
  먼저 보여줍니다.
- **버퍼가 넘치는 걸 알려주지 않던 문제** — 메모리 버퍼는 2만 줄 링이라 넘치면 **앞부터**
  버립니다. 그런데 요약줄은 그냥 `20000줄`이라고만 적혀서, 재현을 끝내고 로그를 열어 보면
  사고 시점이 이미 사라져 있습니다. 실측: MessengerBot이 돌고 있는 TB373FU에서 레벨 V로
  수집하면 **초당 400~1,000줄**, 즉 30초~1분이면 버퍼가 한 바퀴 돕니다. `/api/logcat`가
  `capacity`를 돌려주고, 가득 차면 `버퍼 가득 · 오래된 로그 삭제 중`을 띄워
  `[파일로 저장]`을 권합니다(파일로 받는 중이면 띄우지 않습니다).

검증 232개 → **259개**, 4스위트 전부 통과.
